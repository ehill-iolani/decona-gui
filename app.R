...
ui <- dashboardPage(skin = "red",
  ...
  dashboardSidebar(sidebarMenu(
    ...
    menuItem("Quality Control", tabName = "qc", icon = icon("check-square")),
  ...
  tabItems(
    ...
    tabItem(tabName = "qc",
            fluidRow(
              box(
                title = "Quality Control Viz", status = "primary", solidHeader = TRUE, width = 6,
                plotlyOutput("qcviz"),
                downloadButton("downloadqc", "Download QC Report"),
                actionButton("qchelp", "Help!")),
              box(
                title = "Download QC Report", status = "primary", solidHeader = TRUE, width = 6,
                DTOutput("qctable"),
                downloadButton("downloadqctable", "Download QC Table")))),
...
server <- shinyServer(function(input, output, session) {
  ...
  observeEvent(input$run, {
    ...
    # Make rel_abund numeric and an accurate percentage
    condense$rel_abund <- as.numeric(condense$rel_abund)
    condense$rel_abund <- condense$rel_abund * 100

    # Display the relative abundance plot
    relabres <- ggplotly(ggplot(condense, aes(x = barcode, y = rel_abund, fill = gensp)) +
                  geom_bar(stat = "identity") +
                  theme(axis.text.x = element_text(angle = 0, hjust = 1)) +
                  labs(x = "Barcode", y = "Relative Abundance (%)", fill = "Species") +
                  theme_classic())

    output$relabres <- renderPlotly(relabres)

    # Prepare figure for download
    output$downloadfig <- downloadHandler(
      filename = function() {
        paste("decona_relative_abundance_plot.html")
      },
      content = function(file) {
        saveWidget(relabres, file, selfcontained = TRUE)
      }
    )
    ...
    rarefaction <- ggplotly(ggplot(rarefaction, aes(x = reads, y = unique_species, color = barcode)) +
      geom_point() +
      geom_line() +
      labs(x = "Number of Reads", y = "Number of Unique Species", color = "Barcode") +
      theme_classic())

    output$rarefaction <- renderPlotly(rarefaction)

    # Prepare rarefaction curve for download
    output$downloadrarefaction <- downloadHandler(
      filename = function() {
        paste("decona_rarefaction_curve.html")
      },
      content = function(file) {
        saveWidget(rarefaction, file, selfcontained = TRUE)
      }
    )
    ...
    bc_names <- dir()[str_detect(dir(), "barcode[0-9]+")]

    # Get the number of reads per barcode
    read_counts <- data.frame()
    for (i in bc_names) {
      bctemp <- i
      raw <- readLines(file.path(i, paste0(i, ".fastq")))
      raw <- length(raw) / 4
      filtered <- readLines(file.path(i, paste0(i, ".fastq_Q10_filt.fastq")))
      filtered <- length(filtered) / 4
      temp <- cbind(bctemp, raw, filtered)
      read_counts <- rbind(read_counts, temp)
    }

    # Clean up barcode names
    read_counts$bctemp <- str_remove_all(read_counts$bctemp, "_concatenated")
    names(read_counts) <- c("Barcode", "Raw", "Filtered")
    read_counts$Raw <- as.numeric(read_counts$Raw)
    read_counts$Filtered <- as.numeric(read_counts$Filtered)

    # Plot the data
    qc_report <- ggplotly(ggplot(read_counts, aes(x = Barcode)) +
      geom_bar(aes(y = Raw, fill = "Raw"), stat = "identity") +
      geom_bar(aes(y = Filtered, fill = "Filtered"), stat = "identity") +
      theme_classic() +
      labs(title = "Raw vs Filtered Reads", x = "Barcode", y = "Reads", fill = "Read Catagory") +
      scale_fill_manual(values = c("Raw" = "grey", "Filtered" = "green")))

    output$qcviz <- renderPlotly(qc_report)

    # Create a QC report table
    output$qctable <- renderDT({
      datatable(read_counts, rownames = FALSE)
    })

    # Prepare QCviz report for download
    output$downloadqc <- downloadHandler(
      filename = function() {
        paste("decona_qc_report.html")
      },
      content = function(file) {
        saveWidget(qc_report, file, selfcontained = TRUE)
      }
    )

    # Prepare QC table for download
    output$downloadqctable <- downloadHandler(
      filename = function() {
        paste("decona_qc_table.tsv")
      },
      content = function(file) {
        write.table(read_counts, file, sep = "\t", row.names = FALSE, quote = FALSE)
      }
    )

    #############################################
    ### END OF DECONA CLASSIFIER + VISUALIZER ###
    #############################################
  ...
  observeEvent(input$qchelp, {
    shinyalert(
      title = "Quality Control Help",
      html = TRUE,
      text = "<b>The Quality Control tab displays the number of raw and filtered reads for each barcode based on a minimum quality score and a maximum/minimum read length set by the user.</b><br><br>
      The Quality Control tab is populated by the Decona Classifier + Visualizer workflows.<br><br>",
      type = "info",
      showConfirmButton = TRUE,
      confirmButtonText = "OK",
      confirmButtonCol = "#000000"
